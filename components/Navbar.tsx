    ...
    <>
      <nav className="fixed top-0 left-0 w-full p-6 flex justify-between items-center z-50 pointer-events-none">
        ...
        <div className="flex gap-4 pointer-events-auto">
          <button
            onClick={onToggle}
            className="p-3 backdrop-blur-md rounded-xl border border-white/10 bg-white/5 text-white hover:bg-white/10 transition-all group"
            aria-label="Menu"
          >
            {isOpen ? (
      ...
      <div
        className={`fixed inset-0 z-40 flex flex-col items-center justify-center transition-all duration-500 ease-in-out bg-black/95 ${
        ...
        <div className="flex flex-col items-center space-y-6">
          {menuItems.map((item, idx) => (
            <a
              ...
              className="text-3xl md:text-5xl lg:text-7xl font-black uppercase tracking-tighter text-white/40 hover:text-white transition-all duration-300 hover:scale-110 relative group"
            >
              <span className="relative z-10">{item.label}</span>
              <span className="absolute inset-0 blur-2xl rounded-full transition-all duration-300 group-hover:bg-white/5"></span>
...
const SocialIcon: React.FC<{ icon: React.ReactNode; href: string }> = ({ icon, href }) => (
  <a
    ...
    className="p-3 rounded-full bg-white/5 text-white/80 hover:bg-white/20 hover:text-cyan-400 transition-all transform hover:-translate-y-1"
  >
    {icon}
  </a>
);
